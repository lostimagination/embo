...
/**
 * Start of hidden wrap div for product quantity with sold individually attribute
 */
function it_add_hidden_wrap_start() {
	global $product;
	if ( 'yes' === $product->get_sold_individually() ) {
		echo '<div class="quantity-hidden">';
	}
}
add_action( 'woocommerce_before_add_to_cart_quantity', 'it_add_hidden_wrap_start', 20 );

/**
 * End of hidden wrap div for product quantity with sold individually attribute
 */
function it_add_hidden_wrap_end() {
	global $product;
	if ( 'yes' === $product->get_sold_individually() ) {
		echo '</div>';
	}
}
add_action( 'woocommerce_after_add_to_cart_quantity', 'it_add_hidden_wrap_end', 20 );
...
function it_header_add_to_cart_fragment( $fragments ) {
	...
	?>
	<a class="mini-cart <?php echo ( 0 === $count ) ? 'mini-cart--empty' : ''; ?>" href="<?php echo wc_get_cart_url(); ?>" aria-label="<?php echo esc_attr( $count ); ?>">
		<svg>
			<use xlink:href="#cart"></use>
		</svg>
		<?php if ( 0 !== $count ) : ?>
			<span class="mini-cart__total" role="presentation"><?php echo $count; ?></span>
		<?php endif; ?>
	</a>
	<?php
